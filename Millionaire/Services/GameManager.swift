...
//    MARK: - Init
    ...
    func saveGameRuntimeState() {
        guard let session = currentSession, !session.isFinished else { return }
        
        let audioState = audioService.currentState()
        let state = GameRuntimeState(
            session: session,
            remainingTime: timer.remainingSeconds,
            audioState: audioState
        )
        print("Сохряняем \(state.remainingTime)")
        storage.saveGameRuntimeState(state)
    }
